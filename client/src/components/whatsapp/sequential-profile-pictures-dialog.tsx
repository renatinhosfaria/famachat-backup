...
export function SequentialProfilePicturesDialog({
  ...
  const checkStatus = async () => {
    try {
      ...
      const status = response as ProfilePicStatus;
      // Salvar dados do status para uso nos componentes visuais
      setStatusData(status);
      
      // Calcular progresso
      const progress = status.clientsTotal > 0 
        ? Math.round((status.clientsProcessed / status.clientsTotal) * 100) 
        : 0;
      
      // Formatar mensagem de status
      let statusMsg = "";
      if (status.isRunning) {
        statusMsg = `Processando ${status.clientsProcessed} de ${status.clientsTotal} clientes (${progress}%)`;
      } else if (status.isFinished) {
        // Verificar se o processamento foi completo ou interrompido
        const processamentoCompleto = status.clientsProcessed === status.clientsTotal;
        
        statusMsg = processamentoCompleto
          ? `Processamento concluído! ${status.updatedPhotos} fotos atualizadas de ${status.clientsTotal} clientes`
          : `Processamento concluído: ${status.updatedPhotos} fotos atualizadas de ${status.clientsProcessed} processados de ${status.clientsTotal} clientes`;
        ...
        toast({
          ...
          description: processamentoCompleto 
            ? `${status.updatedPhotos} fotos de perfil atualizadas com sucesso`
            : `${status.updatedPhotos} fotos atualizadas (${status.clientsProcessed} de ${status.clientsTotal} clientes processados)`,
        });
      }
      ...
      if (status.startTime) {
        ...
        if (status.elapsedTimeInSeconds > 0) {
          // Formatar duração
          const duration = intervalToDuration({ start: 0, end: status.elapsedTimeInSeconds * 1000 });
        ...
        if (status.isRunning && status.clientsProcessed > 0 && status.clientsTotal > 0) {
          const percentComplete = status.clientsProcessed / status.clientsTotal;
          if (percentComplete > 0) {
            const estimatedTotalSeconds = status.elapsedTimeInSeconds / percentComplete;
            const remainingSeconds = estimatedTotalSeconds - status.elapsedTimeInSeconds;
        ...
        details += `\nFotos atualizadas: ${status.updatedPhotos} de ${status.clientsProcessed} processados`;
      }
                  ...
                  <Card className="bg-orange-50">
                    <CardContent className="pt-4">
                      ...
                      <div className="text-2xl font-bold text-orange-700">
                        {statusData ? (statusData.clientsProcessed - statusData.updatedPhotos) : 0}
                      </div>
                    </CardContent>
                  </Card>
                </div>
                
                {/* Removidos os cards de processamento e última foto atualizada para simplificar a interface */}
                
                {/* Tabela de resultados recentes - exibida apenas quando houver resultados */}
                {whatsappLogs?.some(log => 
